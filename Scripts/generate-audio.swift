...
let melody = [0, 4, 7, 9, 7, 4, 2, 0, 5, 9, 12, 9, 7, 5, 4, 2, 7, 11, 14, 12, 11, 7, 5, 4, 0, 4, 7, 12, 9, 7, 4, 0]
let progression = [[0, 4, 7], [5, 9, 12], [7, 11, 14], [0, 4, 7]]
let styles = [
    TrackStyle(name: "classic", melody: melody, chords: progression, tempo: 104, octaveShift: 0, decay: 1.8, harmonics: [(1, 1), (2, 0.22), (3, 0.10)], swing: 0),
    TrackStyle(name: "piano", melody: melody, chords: progression, tempo: 88, octaveShift: -12, decay: 5.2, harmonics: [(1, 1), (2, 0.35), (3, 0.17), (4, 0.07)], swing: 0),
    TrackStyle(name: "music-box", melody: melody, chords: progression, tempo: 112, octaveShift: 12, decay: 7.5, harmonics: [(1, 1), (3, 0.34), (5, 0.14)], swing: 0),
    TrackStyle(name: "jazz", melody: melody, chords: [[0, 4, 7, 11], [5, 9, 12, 16], [7, 11, 14, 17], [0, 4, 7, 11]], tempo: 116, octaveShift: -5, decay: 3.1, harmonics: [(1, 1), (2, 0.28), (3, 0.09)], swing: 0.16)
]
...
for style in styles {
    ...
    let duration = beat * Double(style.melody.count) * 0.5
    var samples = [Double](repeating: 0, count: Int(duration * Double(sampleRate)))

    for (index, semitone) in style.melody.enumerated() {
        let startBeat = Double(index) * 0.5 + (index.isMultiple(of: 2) ? 0 : style.swing)
        addNote(
            midi: 60 + semitone + style.octaveShift,
    ...
    let totalBeats = Int(ceil(duration / beat))
