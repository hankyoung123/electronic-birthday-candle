...
/// Low-frequency wind detector. iOS Voice Processing / AEC is expected to have
/// removed our own music from the mic beforehand; this detector only scores the
/// residual low-frequency airflow that a real breath produces.
///
/// Single signal path: wind energy (weighted) + wind ratio (soft shape check),
/// fully additive. No adaptive baseline, no delta features, no broadband.
final class BlowDetector: @unchecked Sendable {
    ...
    func analyze(samples: UnsafeBufferPointer<Float>, sampleRate: Double) -> Float {
        ...
        let audible = totalRMS >= config.silenceFloorRMS

        // Wind energy: how much low-frequency airflow there is.
        let windEnergyScore = audible
            ? normalized(wind.windBandRMS, lower: config.windStart, upper: config.windFull)
            : 0
        // Wind ratio: how much of the 80–5000 Hz energy sits below 500 Hz — a
        // soft shape check so ordinary louder sounds don't trigger on volume
        // alone.
        let windRatioScore = audible
            ? normalized(wind.windRatio, lower: config.windRatioStart, upper: config.windRatioFull)
            : 0
